<?php $this->load->view('theme/header_alph');?>
                ...
                </form><a class="btn btn-light action-button" role="button" href="#newparent" data-toggle="modal" style="margin-top: 2px;background: rgb(86, 198, 198);font-size: 12px;border-radius: 0px;border-top-left-radius: 4px;border-bottom-left-radius: 4px;"><i class="fa fa-plus" style="font-size: 14px;"></i>  Nouveau</a><a class="btn btn-light action-button" role="button" href="#filterprintparent" data-toggle="modal" style="margin-top: 2px;background: var(--gray);font-size: 12px;border-radius: 0px;border-top-right-radius: 4px;border-bottom-right-radius: 4px;"><i class="fa fa-print" style="font-size: 14px;"></i></a>
            </div>
        </div>
    </nav>
    <div class="card" style="border-radius: 0px;margin-bottom: 22px;border-width: 0px;">
        <div class="card-body" style="margin-top: 0px;margin-bottom: 3px;">
            ...
            <a class="btn btn-light action-button" role="button" href="#filterparent" data-toggle="modal" style="margin-top: 2px;background: var(--gray);font-size: 12px;border-radius: 4px;"><i class="fa fa-filter" style="font-size: 13px;"></i>  Filtre</a>
			<a class="btn btn-light action-button" role="button" href="<?php echo base_url('pare');?>" style="margin-top: 2px;background: var(--teal);font-size: 12px;border-radius: 4px;"><i class="fa fa-refresh" style="font-size: 13px;"></i></a></h1>
                ...
                <table class="table">
					...
					<?php foreach ($data as $pare) {?>
                        <tr>
							...
							data-site_id="<?php echo $pare->site_id;?>"
							...
							data-parent_nbr_sco_voz="<?php echo $pare->parent_nbr_sco_voz;?>"
							...
							<a class="btn btn-light" 
                            onclick="return confirm('Voulez-vous supprimer parent <?php echo $pare->parent_pere_name;?> et <?php echo $pare->parent_mere_name;?> ?');"
							href="<?php echo base_url('paredelete/'.$pare->id)?>" 
...
<?php $this->load->view('parent/modal_create_pare')?>
<?php $this->load->view('parent/modal_filter')?>
<?php $this->load->view('parent/modal_print_filter')?>
<script src="<?php echo base_url('assets/js/jquery.min.js') ?>"></script>
<script src="<?php echo base_url('assets/bootstrap/js/bootstrap.min.js'); ?>"></script>
<script>
    $(document).ready(function(){

		$('#newparent').on('hidden.bs.modal', function (e) {
  		$(this)
    	.find("input,textarea,select")
       	.val('')
       	.end()
		});
        $('#filterprintparent').on('hidden.bs.modal', function (e) {
  		$(this)
    	.find("input,textarea,select")
       	.val('')
       	.end()
        ...
        $('.btn-edit').on('click',function(){
            ...
            const parent_nbr_sco_voz = $(this).data('parent_nbr_sco_voz');
